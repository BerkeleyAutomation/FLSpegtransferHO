...
q_des = np.load(file_path + 'new_q_des.npy') # desired joint angles: [q0, ..., q6]
q_act = np.load(file_path + 'new_q_act.npy')

file_path = './result/peg_traj_sampled/RNN_forward_peg_traj/'
new_q_des = np.load(file_path + 'new_q_des.npy')   # desired joint angles: [q0, ..., q6]
new_q_act = np.load(file_path + 'new_q_act.npy')   # actual joint angles: [q0, ..., q6]
...
def plot_all(rand=False):
    ...
    q_des_orig = np.load(file_path + 'wo_model/new_q_des.npy')  # desired joint angles: [q0, ..., q6]
    pos_des_orig = np.array(convert_to_cartesian(q_des_orig))
    q_act_orig = np.load(file_path + 'wo_model/new_q_act.npy')  # desired joint angles: [q0, ..., q6]
    ...
    for model, model_name, color in zip(models, model_names, colors):
        try:
            q_act = np.load(file_path + model + '/new_q_act.npy')  # actual joint angles: [q0, ..., q6]
