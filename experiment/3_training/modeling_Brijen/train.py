...
def load_data(data_dir):
    print (osp.join(data_dir, "q_act.npy"))
    # import pdb; pdb.set_trace()
    joint_actual = np.load(osp.join(data_dir, "q_act.npy"))
    joint_desired = np.load(osp.join(data_dir, "q_des.npy"))
    position_actual = np.load(osp.join(data_dir, "pos_act.npy"))
    position_desired = np.load(osp.join(data_dir, "pos_des.npy"))
    quaternion_actual = np.load(osp.join(data_dir, "quat_act.npy"))
    quaternion_desired = np.load(osp.join(data_dir, "quat_des.npy"))
    return {
...
def format_data(H, fname, use_actual_inputs=False, rnn=False):
    ...
    desired = data["joint_desired"][:,3:]   # q4,q5,q6
...
def create_config():
    ...
    """
    config = DotMap()
    config.peg_data = "../peg_transfer/filtered_downsampled"
    config.random_data = "../random/filtered_downsampled"
