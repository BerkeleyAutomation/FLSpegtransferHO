...
class dvrkMotionBridgeM():
    ...
    def run(self):
        while True:
            ...
            act_motor_current1 = self.dvrk.arm1._dvrkArm__act_motor_current
            act_motor_current2 = self.dvrk.arm2._dvrkArm__act_motor_current
            concat = list(act_pos1) + list(act_rot1) + list(act_jaw1)\
                     + list(act_pos2) + list(act_rot2) + list(act_jaw2)\
                     + list(act_joint1) + list(act_joint2) + list(act_motor_current1) + list(act_motor_current2)
    ...
    def main(self):
        while True:
            ...
            if method==0:
                self.dvrk.set_pose(pos1=des_pos1, rot1=des_rot1, pos2=des_pos2, rot2=des_rot2, wait_callback=False)
                # self.dvrk.set_pose_interpolate(pos1=des_pos1, rot1=des_rot1, pos2=des_pos2, rot2=des_rot2)
                self.dvrk.set_jaw(jaw1=des_jaw1, jaw2=des_jaw2)
            elif method==1:
                self.dvrk.set_joint(joint1=des_joint1, joint2=des_joint2)
                # self.dvrk.set_joint_interpolate(joint1=des_joint1, joint2=des_joint2)
                self.dvrk.set_jaw(jaw1=des_jaw1, jaw2=des_jaw2)
            elif method==2:
