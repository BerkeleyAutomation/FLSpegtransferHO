...
from FLSpegtransfer.trash.CubicOptimizer_1wp_ import CubicOptimizer_1wp_
...
class dvrkPegTransferMotionHandOver:
    ...
    def __init__(self, use_controller, use_optimization, optimizer):
        ...
        if self.optimizer == 'cubic':
            ...
            self.motion_opt_1wp_PSM1 = CubicOptimizer_1wp(max_vel=dvrkVar.v_max, max_acc=dvrkVar.a_max,
                                                     t_step=0.01, print_out=False, visualize=False)
            self.motion_opt_1wp_PSM2 = CubicOptimizer_1wp_(max_vel=dvrkVar.v_max, max_acc=dvrkVar.a_max,
                                                          t_step=0.01, print_out=False, visualize=False)
        ...
        self.offset_grasp = {'PSM1': [-0.004, +0.003], 'PSM2': [-0.005, +0.003]}
    ...
    def handover_ready(self, which_arm='PSM1'):
        if which_arm=='PSM1':
            self.move_upright(pos=[0.120, -0.01, self.height_ready_handover],
                              rot=0.0, jaw='open', which_arm=which_arm)
        elif which_arm=='PSM2':
            self.move_upright(pos=[-0.110, -0.01, self.height_ready_handover],
                              rot=0.0, jaw='open', which_arm=which_arm)
        else:
            raise ValueError
    ...
    def pick_block(self, pos, rot, which_arm='PSM1'):
        ...
        t1 = threading.Thread(target=obj.set_pose_interpolate, args=([pos[0], pos[1], pos[2]+self.offset_grasp[which_arm][1]],
        ...
        obj.set_pose_interpolate(pos=[pos[0], pos[1], pos[2]+self.offset_grasp[which_arm][0]],
                     rot=U.euler_to_quaternion(np.deg2rad([rot, 0.0, 0.0])))
        obj.set_jaw_interpolate(jaw=[self.jaw_closing[index]])

    def go_pick_traj(self, obj, obj_opt, pos_pick, rot_pick, optimized, optimizer, which_arm):
        self.lock.acquire()
        if optimized:
            # trajectory to pick
            ...
            qf = dvrkKinematics.pose_to_joint(pos=[pos_pick[0], pos_pick[1], pos_pick[2] + self.offset_grasp[which_arm][1]],
                                              rot=U.euler_to_quaternion(np.deg2rad([rot_pick, 0.0, 0.0])))
            if optimizer == 'cubic':
            ...
            posf = [pos_pick[0], pos_pick[1], pos_pick[2] + self.offset_grasp[which_arm][1]]
    ...
    def go_pick(self, pos_pick, rot_pick, which_arm):
        if which_arm == 'PSM1':
            ...
            raise ValueError
        q_pos = self.go_pick_traj(obj, obj_opt, pos_pick, rot_pick, self.use_optimization, self.optimizer, which_arm)
    ...
    def go_handover_traj(self, obj_opt, pos_pick, rot_pick, pos_place, rot_place, optimized, optimizer, which_arm):
        self.lock.acquire()
        if optimized:
            # trajectory to transferring block from peg to peg
            q0 = dvrkKinematics.pose_to_joint(pos=[pos_pick[0], pos_pick[1], pos_pick[2] + self.offset_grasp[which_arm][0]],
                                              rot=U.euler_to_quaternion(np.deg2rad([rot_pick, 0.0, 0.0])))
            qw = dvrkKinematics.pose_to_joint(pos=[pos_pick[0], pos_pick[1], self.height_ready],
                                               rot=U.euler_to_quaternion(np.deg2rad([rot_pick, 0.0, 0.0])))
            ...
            if optimizer == 'cubic':
                ...
                x, H = self.motion_opt.optimize_lift_to_handover_motion(q0[0], qw[0], qf[0])
                if x is not None:
                    dim = 6
                    q_pos = np.array([x[t * dim:(t + 1) * dim] for t in range(H + 1)])
        else:
            # trajectory to transferring block from peg to peg
            pos0 = [pos_pick[0], pos_pick[1], pos_pick[2] + self.offset_grasp[which_arm][0]]
    ...
    def go_handover(self, pos_pick, rot_pick, pos_place, rot_place, which_arm):
        ...
        # go down toward block & close jaw
        obj.set_pose_interpolate(pos=[pos_pick[0], pos_pick[1], pos_pick[2]+self.offset_grasp[which_arm][0]],
                                       rot=U.euler_to_quaternion(np.deg2rad([rot_pick, 0.0, 0.0])))
        obj.set_jaw_interpolate([self.jaw_closing[index]])

        # pick-up and handover
        q_pos = self.go_handover_traj(obj_opt, pos_pick, rot_pick, pos_place, rot_place, self.use_optimization, self.optimizer, which_arm)
        if which_arm == 'PSM1':
            t1 = threading.Thread(target=self.move_trajectory, args=(obj, q_pos))
        elif which_arm == 'PSM2':
            t1 = threading.Thread(target=self.move_trajectory2, args=(obj, q_pos))
    ...
    def servoing_block(self, pos_place, rot_place, which_arm):
        ...
        obj.set_pose_interpolate(pos=[pos_place[0], pos_place[1], self.height_ready],
                                       rot=U.euler_to_quaternion(np.deg2rad([rot_place, 0.0, 0.0])))
