...
class dvrkPegTransferMotionDualArm:
    ...
    def __init__(self, use_controller, use_optimization):
        ...
        self.jaw_closing = np.deg2rad([-10, -10])
        ...
        self.offset_grasp = [-0.004, +0.002]
        self.height_ready = -0.123
    ...
    def go_pick(self, pos_pick1, rot_pick1, pos_pick2, rot_pick2):
        q_pos1 = self.go_pick_traj(self.arm1, pos_pick1, rot_pick1, self.use_optimization)
        q_pos2 = self.go_pick_traj(self.arm2, pos_pick2, rot_pick2, self.use_optimization)
    ...
    def transfer_block(self, pos_pick1, rot_pick1, pos_place1, rot_place1, pos_pick2, rot_pick2, pos_place2, rot_place2):
        ...
        q_pos1 = self.transfer_block_traj(pos_pick1, rot_pick1, pos_place1, rot_place1, self.use_optimization)
        q_pos2 = self.transfer_block_traj(pos_pick2, rot_pick2, pos_place2, rot_place2, self.use_optimization)
        t1 = threading.Thread(target=self.move_trajectory, args=(self.arm1, q_pos1))
        t2 = threading.Thread(target=self.move_trajectory, args=(self.arm2, q_pos2))
        t3 = threading.Thread(target=self.arm1.set_jaw_interpolate, args=[[self.jaw_closing[0]]])
        t4 = threading.Thread(target=self.arm2.set_jaw_interpolate, args=[[self.jaw_closing[1]]])
    ...
    def return_to_peg(self, pos_pick1, rot_pick1, pos_pick2, rot_pick2):
        # return to peg
        q_pos1 = self.return_to_peg_traj(self.arm1, pos_pick1, rot_pick1, self.use_optimization)
        q_pos2 = self.return_to_peg_traj(self.arm2, pos_pick2, rot_pick2, self.use_optimization)
        t1 = threading.Thread(target=self.move_trajectory, args=(self.arm1, q_pos1))
        t2 = threading.Thread(target=self.move_trajectory, args=(self.arm2, q_pos2))
        t3 = threading.Thread(target=self.arm1.set_jaw_interpolate, args=[[self.jaw_closing[0]]])
        t4 = threading.Thread(target=self.arm2.set_jaw_interpolate, args=[[self.jaw_closing[1]]])
