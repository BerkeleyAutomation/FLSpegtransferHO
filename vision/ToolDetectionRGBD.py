...
class ToolDetectionRGBD:
    def __init__(self, Trc):
        ...
        # thresholding value
        self.__lower_red = np.array([0-20, 40, 40])
    ...
    def find_tool(self, img_color, img_point, color, visualize=False):
        if visualize:
        ...
        cnts_needle = [c for c in cnts if cv2.contourArea(c) > 20]  # threshold by contour size
        if visualize:
        ...
        if visualize:
            ...
            cv2.circle(img_color_copy, (int(pt[1]), int(pt[0])), 3, (0, 255, 255), -1)
            cv2.imshow("", img_color_copy)
            cv2.waitKey(0)
        return img_point[pt[0], pt[1]]