...
from FLSpegtransfer.utils.ICP.affineICP import affineICP
...
class NeedleTrackingStereoEllipse:
    ...
    def find_needle3D(self, img_left, img_right, color, visualize=False):
        ...
        pnts_needle_L = self.get_coordinate(img_nd_masked_L_ds)
        pnts_needle_R = self.get_coordinate(img_nd_masked_R_ds)
        ...
        # init pose
        t_init = np.mean(pnts_needle_R.T, axis=0) - np.mean(pnts_needle_L.T, axis=0)
        T_init = np.eye(3)
        T_init[:2, -1] = t_init
        T, distances, iterations = affineICP.icp(pnts_needle_L.T, pnts_needle_R.T, init_pose=T_init)
        # pnts_cln_R_new = TY.T
        ...
        R = T[:2,:2]
        t = T[:2, -1]
        pnts_cln_R_new= R.dot(pnts_needle_L) + t.reshape(2,1)

        img_left_copy = img_left.copy()
        img_right_copy = img_right.copy()
        img_left_copy = self.overlay_pnts(img_left_copy, pnts_needle_L, pnts_needle_L)
        img_right_copy = self.overlay_pnts(img_right_copy, pnts_cln_R_new, pnts_needle_R)
